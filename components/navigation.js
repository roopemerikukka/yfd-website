...
import { BEZIER_SLOWER } from '../common/bezierFunctions'
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <Observer>
          {({ inView, ref }) => (
            <nav className={`${navClasses} is-${inView ? 'visible' : 'hidden'}`} ref={ref}>
              <div className='nav__close' onClick={() => this.menuToggle()}>
                <span></span>
                <span></span>
              </div>
              <Link scroll={false} to='/'>
              ...
              <ul className='nav__links'>
                {navLinks.map(( navigationLink, index ) => {
                  const isActiveNav = ( currentPath === navigationLink.fields.slug ) || ( currentPath === '' && navigationLink.fields.slug === 'home' )
                  return (
                    <li key={index}>
                      <Link prefetch scroll={false} to={navigationLink.fields.slug === 'home' ? '/' : `/${navigationLink.fields.slug}`}>
                        <a className={`${textStyles.navLinks.className} ${isActiveNav ? 'is-active' : ''}`} onClick={() => { this.menuToggle() }}>{navigationLink.fields.title}</a>
                      </Link>
                    </li>
                  )
                }
        ...
        <style jsx>{`
          @media screen and (min-width: ${breakpoints.large}) {
            ...
            .nav.is-toggled {
              ...
              opacity: 0;
              transition: opacity 0.8s ${BEZIER_SLOWER};
            }
